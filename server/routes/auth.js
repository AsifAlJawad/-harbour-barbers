...
router.get('/me', requireAuth, async (req, res) => {
  try {
    const { data: row, error } = await supabase.from('profiles').select('*').eq('id', req.user.id).maybeSingle()
...
router.put('/profile', requireAuth, async (req, res) => {
  ...
  try {
    // Build update object with only defined fields (preserve existing values for undefined ones)
    const updates = {}
